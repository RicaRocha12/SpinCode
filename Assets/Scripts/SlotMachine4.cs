...
public class SlotMachine4 : MonoBehaviour
{
    [Header("Botões")]
    ...
    public static bool reiniciouTotalmente = false;

    void Start()
    {
        if (reiniciouTotalmente)
        {
            vidas = 2;
            reiniciouTotalmente = false; 
        }
        else
        {
            vidas = PlayerPrefs.GetInt($"Nivel1_Vidas", 1) + 1;
        }
        ...
        botãoSair.onClick.AddListener(() => SceneManager.LoadScene("PaginaIncial")); // Botão para sair do jogo
    ...
    void CarregarPerguntas()
    {
        ...
        TextAsset ficheiro = Resources.Load<TextAsset>("perguntas4");
        Debug.Log("Conteúdo do ficheiro:\n" + ficheiro.text);
        if (ficheiro == null)
        {
            Debug.LogError("Ficheiro perguntas4.txt não encontrado em Resources!");
            return;
        }
    ...
    {
        if (apostaDoJogador.Count >= 4 || isSpinning) return;
    ...
    IEnumerator SpinReels()
    {
        ...
        if (acertouTudo)
        {
            ...
            // Se acertou 4 perguntas, guarda progresso e muda para a cena "nivel3"
            if (perguntasRespondidas.Count >= 4)
            {
                ...
                PlayerPrefs.SetString("Nivel2_PerguntasCorretas", perguntasCorretas);
                PlayerPrefs.SetInt("Nivel2_Vidas", vidas);
                PlayerPrefs.Save();
                Debug.Log("Progresso guardado em PlayerPrefs (WebGL)");
                SceneManager.LoadScene("Nivel3"); // Muda para a cena "nivel3"
    ...
    {
        reiniciouTotalmente = true; // Marca que foi reiniciado totalmente
