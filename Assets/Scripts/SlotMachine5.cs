...
public class SlotMachine5 : MonoBehaviour
{
    [Header("Botões")]
    ...
    void Start()
    {
        vidas = PlayerPrefs.GetInt($"Nivel2_Vidas", 1) + 1; // Pega o número de vidas guardadas ou define como 3
    ...
    void CarregarPerguntas()
    {
        ...
        TextAsset ficheiro = Resources.Load<TextAsset>("perguntas5");
    ...
    {
        if (apostaDoJogador.Count >= 5 || isSpinning) return;
    ...
    IEnumerator SpinReels()
    {
        ...
        if (acertouTudo)
        {
            ...
            if (perguntasRespondidas.Count >= 4)
            {
                ...
                PlayerPrefs.SetString("Nivel3_PerguntasCorretas", perguntasCorretas);
                PlayerPrefs.SetInt("Nivel3_Vidas", vidas);
                PlayerPrefs.Save();
                Debug.Log("Progresso guardado em PlayerPrefs (WebGL)");
                SceneManager.LoadScene("Vitoria"); // Muda para a cena "nivel4"
